...
holdsS (Sta containers n) container ruta = inOrderR ruta (destinationC container) (destinationC (last containers) )  
                                           && (netS (Sta (container:containers) n) <= 20) -- por eninciado el peso maximo es 20 
                                           && (freeCellsS (Sta (container:containers) n) >= 0)
