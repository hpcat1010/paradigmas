...
holdsS (Sta containers n) container ruta = inOrderR ruta (destinationC container) (destinationC (last containers) )  -- en duda q significa "si la pila puede aceptar el contenedor considerando las ciudades en la ruta"
...
popS (Sta containers n) ciudad = Sta (filter (\container -> destinationC container /= ciudad) containers) n -- filter takes a predicate and a list and returns the list of elements that satisfy the predicate.
